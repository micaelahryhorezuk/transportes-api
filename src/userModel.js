...
const requiredfields = ['username', 'password', 'name', 'lastname', 'email', 'admin'];

function login(username, password) {
    return new Promise(async (resolve, reject) => {
        if (!username || username == '' || !password || password == '') return reject("Invaalid request.");
        const query = `select * from ${tablename} where username=? and password=? limit 1`;
        const items = await pool.query(query, [username, md5(password)]);
        if (Array.isArray(items) && items.length == 1) resolve(items[0]);
        return reject("Error al iniciar sesion");
    });
}
